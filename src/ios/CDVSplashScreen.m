...
- (CDV_iOSDevice) getCurrentDevice
{
    ...
    device.iPhoneXsMax = (device.iPhone && limit == 1242.0);
...
- (NSString*)getImageName:(UIInterfaceOrientation)currentOrientation delegate:(id<CDVScreenOrientationDelegate>)orientationDelegate device:(CDV_iOSDevice)device
{
    ...
    if ([imageName isEqualToString:@"LaunchImage"])
    {
        ...
        } else if(device.iPhone6Plus || device.iPhoneX || device.iPhoneXs ) {
    ...
    { // does not support landscape
        imageName = [imageName stringByAppendingString:@"-667h"];
         ...
         imageName = [imageName stringByAppendingString:@"-1242h"];
    }
    else if (device.iPhoneXr){
        imageName = [imageName stringByAppendingString:@"-896h"];
    }
    ...
    { // supports landscape
        ...
        if (device.iPhoneX || device.iPhoneXs) {
